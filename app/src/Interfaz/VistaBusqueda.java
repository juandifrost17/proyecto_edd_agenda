...
public class VistaBusqueda extends VBox {
    ...
    private String estiloTextField(boolean focused) {
        ...
        return "-fx-background-color: white; -fx-border-color: " + borde + ";"
                + "-fx-border-radius: 6; -fx-background-radius: 6; -fx-padding: 10 15; -fx-font-size: 14;";
    }

    private String estiloBtnBuscar(boolean hover) {
        String fondo = hover ? "#2471a3" : "#2980b9";
    ...
    public VistaBusqueda() {
        ...
        Label titulo = new Label("Buscar Contactos");
        titulo.setFont(Font.font("Segoe UI", FontWeight.BOLD, 22));
        titulo.setStyle("-fx-text-fill: #2c3e50;");
        Label desc = new Label("Busque por prefijo de nombre, apellido o apodo. Los resultados se ordenan por frecuencia.");
        ...
        txtPrefijo.setPromptText("Escriba un prefijo...");
        txtPrefijo.setPrefWidth(320);
        txtPrefijo.setStyle(estiloTextField(false));
        txtPrefijo.focusedProperty().addListener((obs, old, focused) ->
                txtPrefijo.setStyle(estiloTextField(focused))
        );
        ...
        contenedorResultados.setPadding(new Insets(0));
    ...
    private void ejecutarBusqueda() {
        ...
        LinkedList<Contacto> resultados = AppJavaFX.getAgendaCompartida().buscarPorPrefijoConFrecuencia(prefijo);
        if (resultados.isEmpty()) {
            actualizarEstado("#e67e22", "Sin resultados para \"" + prefijo + "\".");
            return;
        }
        actualizarEstado("#27ae60", resultados.size() + " contacto(s) encontrado(s).");
        int i = 1;
